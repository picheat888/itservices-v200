# Stock Item View Details — Tabbed Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rebuild the Stock Item View Details modal into a 1100px tabbed focus dialog (Overview / Movements / Lots / Serials) matching the Contract View, with status + serialized badges in the header, a fill-height Movements table, and an Edit action in the footer.

**Architecture:** Reuse the shared `DataTable` `fillHeight` mode (already shipped) for the Movements tab. Keep the always-mounted `open={itemId !== null}` pattern (so close animates), and retain the last item so content doesn't flash to skeleton during fade-out. Overview/Lots/Serials read from `useStockItem`; Movements reads from `useStockItemHistory` (same cache key dedupes the count fetch). No backend changes.
...
- TypeScript; PascalCase components; comment every function you create.
- No dependency / backend / API / DB changes.
- No frontend test runner — verify each task with `npx tsc --noEmit` (paste the literal output filtered to the touched file) + the manual checks named. Zero errors in touched files; pre-existing errors elsewhere are out of scope.
- Staging discipline: working tree has unrelated uncommitted work. Each task stages ONLY its own file(s) by explicit path — NEVER `git add -A` / `git add .`.
- New ad-hoc labels (tab names, movement columns, "full history", "Edit") use inline `lang === 'th' ? … : …` (matching this modal's existing pattern) — do NOT add i18n keys (`i18n.ts` is shared with unrelated uncommitted work). Reuse existing `t('stock_*')` keys where the current modal already uses them.
- Reuse the already-shipped `DataTable` `fillHeight` prop — do NOT modify `data-table.tsx`.

---

### Task 1: Stock Movements tab component

A fill-height `DataTable` of the item's stock movements.

**Files:**
- Create: `resources/js/components/stock/stock-movements-tab.tsx`

**Interfaces:**
- Consumes: `useStockItemHistory(id)` from `@/hooks/use-stock` (returns `StockItemHistory` with `.movements[]`); `DataTable` + `Column<T>` from `@/components/shared/data-table`; `StatusBadge`; `StockMovementType` from `@/types`.
- Produces: `StockMovementsTab({ itemId: number })`.
...
Run: `npx tsc --noEmit`
Expected: zero errors referencing `stock-movements-tab.tsx`. (`Column` is exported from `data-table.tsx`; `useStockItemHistory` from `use-stock.ts`; `StockMovementType` from `@/types`.)

- [ ] **Step 3: Commit**

```bash
git add resources/js/components/stock/stock-movements-tab.tsx
git commit -m "feat(stock): movements tab — fill-height table of item movements"
```

---

### Task 2: Rebuild StockItemDetailModal as a tabbed 1100px dialog

**Files:**
- Modify (rewrite): `resources/js/components/stock/stock-item-detail-modal.tsx`

**Interfaces:**
- Consumes: `StockMovementsTab` (Task 1); `useStockItem`, `useStockItemHistory` from `@/hooks/use-stock`.
- Produces: `StockItemDetailModal({ itemId, onClose, onEdit? })` — adds optional `onEdit?: (item: StockItem) => void`.
...
Run: `npx tsc --noEmit`
Expected: zero errors referencing `stock-item-detail-modal.tsx`. Confirm `t('edit')` exists (used widely elsewhere). The removed `History`-button-in-header and the old `Box`/`ShieldCheck` header pills are replaced; ensure no unused imports remain.

- [ ] **Step 3: Manual verification**

Open a stock item from the Stock Items table:
- 1100px dialog; header shows Package icon + name + SKU chip + serialized/qty-only pill + status badge top-right by ✕.
- Tabs open on Overview; Lots/Serials counts correct; Movements count appears (history loads); Serials tab is ABSENT for a qty-only item.
- Overview: KPI strip + meta grid + warehouse cards.
- Movements: table fills + pages (no inner scroll / picker); qty colored by direction; empty/loading states.
- Lots: rows expand one at a time (unchanged behavior); Pager when >10.
- Serials: table + statuses; Pager when >10.
- Footer: "Full history" opens `/stock/items/{id}/history` in a new tab; Edit shows only when `onEdit` provided (wired in Task 3).
- Closing the dialog fades out without flashing the skeleton.

- [ ] **Step 4: Commit**

```bash
git add resources/js/components/stock/stock-item-detail-modal.tsx
git commit -m "feat(stock): tabbed 1100px Stock View Details (Overview/Movements/Lots/Serials)"
```

---

### Task 3: Wire the Edit action from the Stock page

**Files:**
- Modify: `resources/js/pages/stock/index.tsx`

**Interfaces:**
- Consumes: `StockItemDetailModal`'s new `onEdit` prop (Task 2). Existing page state: `viewId`/`setViewId`, `editItem`/`setEditItem`, `canManage`.

- [ ] **Step 1: Pass `onEdit` to the detail modal**

Find the render (around line 676):

```tsx
                <StockItemDetailModal itemId={viewId} onClose={() => setViewId(null)} />
```

Replace with (close the detail and open the existing edit form; gated by `canManage`):
...
Run: `npx tsc --noEmit`
Expected: zero errors referencing `pages/stock/index.tsx`. (`setEditItem` accepts a `StockItem`; the modal passes its loaded `item`.)

- [ ] **Step 3: Manual verification**

As a user with `stock.manage_items` (or super): open an item → the footer shows **Edit** → clicking it closes the detail and opens the `StockItemModal` edit form pre-filled. As a user without the permission: the Edit button is absent.

- [ ] **Step 4: Commit**

```bash
git add resources/js/pages/stock/index.tsx
git commit -m "feat(stock): open edit form from View Details footer (perm-gated)"
```
...
**Spec coverage:**
- 1100px tabbed shell + header (icon, name, SKU chip, serialized pill, status badge top-right) → Task 2. ✓
- Tabs Overview/Movements/Lots/Serials, counts, open-on-Overview, Serials hidden when qty-only → Task 2. ✓
- Overview (KPI + meta + warehouse cards) → Task 2. ✓
- Movements fill-height table from `useStockItemHistory` with signed/colored qty + localized type → Task 1 + Task 2 mounts it. ✓
- Lots expandable rows preserved + Pager → Task 2. ✓
- Serials table + Pager → Task 2. ✓
- Footer full-history link + perm-gated Edit → Task 2 (renders) + Task 3 (wires onEdit). ✓
- Retain-last-item for close animation → Task 2. ✓
- No backend changes; DataTable unchanged → respected. ✓

**Placeholder scan:** No TBD/TODO. New labels are inline `lang` ternaries; existing `t('stock_*')`/`t('edit')` keys reused (all present in the current modal / app).

**Type consistency:** `StockMovementsTab({ itemId: number })` (Task 1) matches Task 2's `<StockMovementsTab itemId={itemId} />` (guarded by `itemId != null`). `onEdit?: (item: StockItem) => void` (Task 2) matches Task 3's `onEdit={canManage ? (i) => {…} : undefined}`. `Column`/`DataTable`/`fillHeight` come from the shipped `data-table.tsx`. `useStockItemHistory` shape (`.movements[]`) matches the `Move` type used.
