...
`RequestNotificationTest` +2 (คำขอค้าง → คนตั้งบัญชีได้ bell พร้อม employee_id · แถวคิว IT ไม่ยิง) · `EmployeeApiTest` +1 (คนที่กดเพิ่มพนักงานได้ bell ด้วย) · `SidebarBadgeTest` +1 · **ยืนยันว่าเทสต์จับได้จริงด้วยการปิดโค้ดใหม่ชั่วคราวแล้วเห็น fail ทั้งคู่ ก่อนคืนกลับ**
**ทั้ง suite = 866 passed / 3,361 assertions** · `tsc --noEmit` = 0 · eslint = 0 · prettier ผ่าน · pint ผ่าน · `npm run build` ผ่าน
