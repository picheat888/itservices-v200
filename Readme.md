...
- เพิ่ม `no_matching_position` ทั้งใน TS union + ตาราง label + ข้อความ EN/TH (ต่อมาแยกเป็น 2 รหัส — ดูหัวข้อถัดไป)
...
## แยกเหตุผลการข้ามขั้น: "ผู้ขออยู่ระดับนั้นแล้ว" ≠ "สายไม่มีคนระดับนั้น" (2026-08-07)

`no_matching_position` เดิมกินสองสถานการณ์ที่ต่างกันจริง ๆ แล้วเล่าด้วยประโยคเดียว ("ตำแหน่งเท่ากัน หรือน้อยกว่า") ซึ่งอธิบายผิดในกรณีที่สอง:

| สถานการณ์ | ตัวอย่าง | รหัสใหม่ |
|---|---|---|
| ผู้ขอถือตำแหน่งของขั้นนั้นหรือสูงกว่าอยู่แล้ว | Supervisor ยื่นเอง → ขั้น Supervisor ไม่มีใครเหนือขึ้นไปให้ถาม | `requester_outranks_step` |
| สายบังคับบัญชาไม่มีใครถือตำแหน่งระดับนั้น | Staff รายงานตรงต่อ Manager (แผนกเล็ก ไม่มี Supervisor คั่น) | `no_matching_position` |

- **ตัดสิน "สูงกว่า" จากลำดับ rung ใน workflow เอง** ไม่ใช่จากตาราง `positions` (ซึ่งเก็บแค่ `title` ไม่มีคอลัมน์ระดับ) — `rungOf()` หา rung ที่ตำแหน่งของผู้ขอปรากฏ (เอา rung สูงสุดถ้าซ้ำ) แล้ว `skipReasonFor()` ตัดสินว่าขั้นที่ index ≤ rung นั้น = outranked ส่วนที่เหลือ = สายไม่มีคนถือ · ผู้ขอที่ไม่ปรากฏใน rung ใดเลย (เช่น Staff/Officer) จะได้ `no_matching_position` เสมอ ซึ่งถูกต้อง
- ข้อความใหม่: `ข้ามขั้นนี้ เนื่องจากผู้ขอถือตำแหน่งระดับนี้หรือสูงกว่าอยู่แล้ว` / `ข้ามขั้นนี้ เนื่องจากในสายอนุมัติไม่มีผู้ถือตำแหน่งระดับนี้`
- **แถวเก่าใน DB ไม่ถูกแปลง** (7 แถว ยังเป็น `no_matching_position`) เพราะ snapshot ต้องคงความจริงตอนยื่น — การอนุมานย้อนหลังต้องอ่าน `position_ids` ของ workflow *ปัจจุบัน* ซึ่งอาจถูกแก้ไปแล้วหลังจากนั้น ข้อความที่แถวเก่าแสดงยังไม่ผิด (สายนั้นไม่มีผู้ถือตำแหน่งที่อนุมัติได้จริง) แค่ไม่เจาะจงเท่าใบใหม่
- กรณีที่สาม (rung สองขั้นชื่อตำแหน่งเดียวกัน คนถือเซ็นขั้นล่างไปแล้ว) ยังใช้ `no_matching_position` — ยังเป็นจริงในความหมาย "ไม่มีผู้ถือตำแหน่งนี้ที่ยังไม่ได้เซ็นในสาย" และไม่ได้แยกออกมาเพราะไม่คุ้มกับรหัสที่สาม

### Tests / Verification

`RequestResolutionTest` — เทสต์เดิมที่พิน "Supervisor ยื่นเอง" เปลี่ยนไป assert `RequesterOutranksStep` · เพิ่มเทสต์ใหม่ที่พินทั้งสองด้านในตัวเดียว: (ก) Manager ยื่นเอง → ขั้น Supervisor **และ** Manager = outranked, VP ยังเซ็น (ข) Staff → Manager ข้ามระดับ → ขั้น Supervisor = `no_matching_position` และ Manager ยังหาคนได้
`SkipReasonWordingTest` จับให้เองว่ารหัสใหม่ต้องมี type + label + ข้อความ 2 ภาษา (20 assertions)
**ทั้ง suite = 875 passed / 3,416 assertions** · `tsc --noEmit` = 0 · eslint = 0 · prettier ผ่าน · pint ผ่าน · `npm run build` ผ่าน
