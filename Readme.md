...
**กฎจริงของ `sla_met_pct` คือ "ตัดเคสที่ถูกจัดประเภทเป็นซ่อมอยู่ตอนนี้ออกเสมอ"** (`TicketController::index()` ทำ `$resolvedInWindow(...)->reject($isRepair)` แบบไม่มีเงื่อนไข) — **ไม่ได้ผูกกับว่ามีกฎ `has_repair_rules` อยู่หรือไม่** ทั้งสองเรื่องเป็นคนละคำถาม: `has_repair_rules` ตอบว่า "ผู้ดูแลระบบตั้งเป้าหมายไว้หรือยัง" ส่วนเซตที่ถูกตัดออกจาก `sla_met_pct` ตอบว่า "ตอนนี้มีเคสไหนถูกจัดว่าเป็นงานซ่อมบ้าง" — และการจัดประเภทเคสไม่เคยถูกบังคับให้ต้องมีกฎรออยู่ก่อน (`TicketWorkClassModal` เสนอทั้งสามคลาสเสมอ ไม่ว่ามีกฎหรือไม่ ไม่มีกฎก็แค่ขึ้นคำเตือน) ในทางปฏิบัติเซตที่ถูกตัดออกจึงว่างเปล่าจนกว่าจะมีใครเริ่มจัดประเภท ซึ่ง**มักจะ**ตามหลังการสร้างกฎ แต่ไม่มีอะไรบังคับลำดับนั้น — เคสที่ถูกจัดเป็นซ่อมก่อนมีกฎเป็นสถานการณ์ที่เกิดได้จริง
