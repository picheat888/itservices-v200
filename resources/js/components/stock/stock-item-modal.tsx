...
import { useCategories, useUnits, useVendors, useWarehouses, useWarrantyTypes } from '@/hooks/use-master-data';
...
const empty: StockItemPayload = {
    sku: '', name: '', serial: '', category: '', brand: '', model: '',
    unit: 'unit', cost: 0, current_stock: 0, min_stock: 0, max_stock: 0,
    warehouse: '', supplier: '', warranty: '',
};
...
export function StockItemModal({ open, item, onClose }: { open: boolean; item?: StockItem | null; onClose: () => void }) {
    ...
    useEffect(() => {
        ...
        setForm(
            item
                ? {
                      sku: item.sku, name: item.name, serial: item.serial ?? '', category: item.category ?? '',
                      brand: item.brand ?? '', model: item.model ?? '', unit: item.unit, cost: item.cost,
                      current_stock: item.current_stock, min_stock: item.min_stock, max_stock: item.max_stock,
                      warehouse: item.warehouse ?? '', supplier: item.supplier ?? '', warranty: item.warranty ?? '',
                  }
                : empty,
        );
    }, [open, item]);
                ...
                <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                        <Field label="SKU" required>
                            <Input value={form.sku} onChange={(e) => set('sku', e.target.value)} disabled={!!item} placeholder="SK-NB-003" className="font-mono" />
                        </Field>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <Field label={t('stock_brand')}><Input value={form.brand ?? ''} onChange={(e) => set('brand', e.target.value)} /></Field>
                        <Field label={t('stock_model')}><Input value={form.model ?? ''} onChange={(e) => set('model', e.target.value)} /></Field>
                    </div>
                    <div className="grid grid-cols-4 gap-3">
                        <Field label={t('stock_current')}><Input type="number" value={form.current_stock} onChange={(e) => set('current_stock', +e.target.value)} className="font-mono" /></Field>
                        <Field label="Min"><Input type="number" value={form.min_stock} onChange={(e) => set('min_stock', +e.target.value)} className="font-mono" /></Field>
                        <Field label="Max"><Input type="number" value={form.max_stock} onChange={(e) => set('max_stock', +e.target.value)} className="font-mono" /></Field>
                        <Field label={t('stock_cost')}><Input type="number" value={form.cost} onChange={(e) => set('cost', +e.target.value)} className="font-mono" /></Field>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                    ...
                    <Field label={t('stock_warranty')}>
                        <Select value={form.warranty || undefined} onValueChange={(v) => set('warranty', v)}>
                            <SelectTrigger><SelectValue placeholder="—" /></SelectTrigger>
                            <SelectContent>
                                {warranties.map((w) => <SelectItem key={w.id} value={w.name}>{w.name}</SelectItem>)}
                            </SelectContent>
                        </Select>
                    </Field>
                </div>
