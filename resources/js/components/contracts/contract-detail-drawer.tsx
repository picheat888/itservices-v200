        ...
        <Sheet open={!!contract} onOpenChange={(o) => !o && onClose()}>
            <SheetContent side="right" className="flex w-[620px] flex-col sm:max-w-[620px]">
                ...
                <div className="mt-6 flex-1 space-y-6 overflow-y-auto px-1">
                    <div className="flex flex-wrap gap-2">
                        ...
                        {contract.in_reminder && (
                            <StatusBadge tone="amber">
                                {lang === 'th' ? `หมดอายุใน ${days} วัน` : `Expires in ${days} days`}
                            </StatusBadge>
                        )}
                    </div>
                    ...
                    {/* Notification schedule — display only; automated sending is deferred. */}
                    <div>
                        ...
                        <div className="space-y-2">
                            {[
                                ...
                                <div key={n.d} className="flex items-center gap-3 rounded-md bg-muted/50 px-3 py-2">
                                    <Mail className="h-4 w-4 text-muted-foreground" />
                                    <div className="flex-1 text-sm">
                                        {lang === 'th' ? `แจ้งเตือนก่อน ${n.d} วัน` : `${n.d}-day reminder`}
                                    </div>
                    ...
                    {contract.notes && (
                        <div className="space-y-1">
                            <div className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{lang === 'th' ? 'หมายเหตุ' : 'Notes'}</div>
                ...
                <SheetFooter className="mt-4 flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                        ...
                        {canRenew && !cancelled && (
                            <Button className="flex-1" onClick={() => renew.mutate({ id: contract.id }, { onSuccess: onClose })} disabled={renew.isPending}>
