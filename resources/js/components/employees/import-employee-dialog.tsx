...
export function ImportEmployeeDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
    ...
    const successMsg = success != null
        ? lang === 'th' ? `นำเข้าสำเร็จ ${success} รายการ` : `Imported ${success} employee(s)`
        : '';
            ...
            <DialogContent className="max-w-lg">
                ...
                <div className="space-y-3 py-1">
                    ...
                    <label className="flex cursor-pointer flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border px-4 py-6 text-center hover:bg-accent/50">
                        ...
                        <span className="text-xs text-muted-foreground">{t('import_hint')}</span>
                        <input
                            ref={inputRef}
                            type="file"
                            accept=".csv,text/csv"
                    ...
                    {generalError && (
                        <div className="rounded-lg bg-destructive/10 px-3 py-2 text-sm text-destructive">{generalError}</div>
                    )}
                            ...
                            <div className="max-h-48 overflow-y-auto px-3 py-2">
                                {errors.map((er) => (
                                    <div key={er.row} className="py-0.5 text-xs">
                                        <span className="font-mono font-semibold text-destructive">{t('import_row')} {er.row}:</span>{' '}
