...
export function AddEmployeeDrawer({ open, onClose, employee }: { open: boolean; onClose: () => void; employee?: Employee | null }) {
    ...
    useEffect(() => {
        if (!open) {
            // The component stays mounted between opens — clear the transient
            // warning/confirm flags on close so a leftover manager-loop / org-change
            // dialog can't linger behind (or after) a dismissed form.
        ...
        // Depend on employee?.id (stable), NOT the employee object: a new object
        // reference with the same id on an incidental re-render would otherwise
        // re-run this and wipe transient state (e.g. the manager-loop warning) the
        // instant it appears, making it flash away.
    ...
    /** Inline bilingual literal helper for the edit dialog's hand-over copy. */
    const L = (th: string, en: string) => (lang === 'th' ? th : en);

    // Department accent — derived from the currently selected department's tag,
    // used to tint the edit dialog header, avatar tile and Save button. Falls
    // back to the app brand colour when no department is selected.
    ...
    // ── Shared field blocks, reused by the add Sheet (stepped) and the edit
    // Dialog (sectioned). Behaviour is identical — only the surrounding shell
    // and grouping differ.

    /** Photo upload + crop trigger (avatar, file picker, remove, error/help). */
    const photoBlock = (
    ...
    /** First/last name EN + first/last name TH grids. */
    const nameFields = (
        <>
    ...
    /** Department select — clears the section on change to drop stale options. */
    ...
    /** Section searchable-select — empty until a department is chosen. */
    const sectionField = (
    ...
    /** Manager searchable-select (loop rejection handled on save). */
    const managerField = (
    ...
    // The crop dialog is shared by both shells — lift it out so editing a photo
    // works whether the form is shown as a Sheet (add) or Dialog (edit).
    ...
    // ── Edit mode: a centered, department-tinted "focus dialog" ──────────────
    if (isEdit) {
            ...
            <>
                ...
                >
                    ...
                    >
                        ...
                        {/* Org-change confirmation rendered INLINE inside this same Dialog.
                            A stacked sibling <Dialog> over the edit dialog fought Radix's
                            focus-trap and only flashed on close (never usable) — the exact
                            failure the manager-loop alert above was inlined to avoid. */}
                        {orgConfirm && (
                            <>
                        ...
                        {!orgConfirm && (
                            <>
                                ...
                                <div className="flex max-h-[min(68vh,620px)] flex-col gap-6 overflow-y-auto px-6 py-5">
                                    {/* Manager-loop alert — inline (NOT a stacked modal: a sibling modal over
                                this dialog fights its focus-trap and Radix auto-closes it, so it only flashes). */}
                                    {managerLoop && (
    ...
    // ── Add mode: the existing stepped right Sheet (unchanged) ───────────────
    return (
        <>
            <Sheet
                open={open}
            ...
            <Dialog
                ...
                onOpenChange={(o) => {
                    if (!o) setOrgConfirm(false);
                }}
            >
                <DialogContent className="max-w-md" onInteractOutside={(e) => e.preventDefault()}>
                    <DialogHeader>
                        <div className="flex items-center gap-2.5">
                            <span className="bg-brand/15 text-brand flex h-9 w-9 shrink-0 items-center justify-center rounded-full">
                                <Briefcase className="h-5 w-5" />
                            </span>
