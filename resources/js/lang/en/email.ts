import type { Dict } from '@/lang/types';

export const email: Dict = {
    email_title: 'Email & Notifications',
    email_sub: 'How emails are worded, and what has been sent',
    email_templates: 'Templates',
    email_templates_sub: 'Every email template in the system',
    email_test: 'Send test',
    email_enabled: 'Enabled',
    email_sent_today: 'Sent today',
    email_delivery: 'Delivery rate',
    email_template: 'Template Name',
    email_trigger: 'Trigger',
    email_type: 'Type',
    email_cadence_realtime: 'Real-time',
    email_cadence_daily: 'Daily',
    email_cadence_weekly: 'Weekly',
    email_var_hint: 'Values are filled in automatically when the email is sent.',
    email_edit_preview: 'Edit & preview',
    email_last_sent: 'Last sent',
    email_search: 'Search templates…',
    email_tab_email: 'Email',
    email_tab_notification: 'Notification',
    email_tab_log: 'Logs',
    email_log_sub: 'The full email delivery history',
    email_log_search: 'Search recipient, subject or template…',
    email_log_time: 'Time',
    email_log_template: 'Template',
    email_log_recipient: 'Recipient',
    email_log_reason: 'Reason',
    email_log_sent: 'Sent',
    email_log_failed: 'Failed',
    email_log_skipped: 'No address',
    email_log_no_address: 'no email address',
    email_log_empty: 'Nothing sent yet',
    email_log_content: 'The email',
    email_log_no_content: 'The message was not kept for sends from before this was recorded.',
    email_preview: 'Preview',
    email_insert_var: 'Insert variable',
    email_variables: 'Variables in this email',
    email_subject: 'Subject',
    email_body: 'Body (HTML)',
    email_save: 'Save',
    email_save_hint: 'Save changes (Ctrl+S)',
    email_unsaved: 'Unsaved',
    email_saved: 'Saved',
    email_sent: 'Sent',
    email_discard_title: 'Discard changes?',
    email_discard_text: 'Your unsaved edits will be lost.',
    email_discard_confirm: 'Discard',
    email_empty_title: 'No templates',
    email_empty: 'No email templates yet.',
    email_empty_filtered: 'No templates match your search.',
    email_test_hint: 'Sends what is on screen to your email, saved or not',
    email_reset: 'Reset',
    email_reset_hint: 'Restore this template to its standard content',
    email_reset_unchanged: 'Already matches the standard',
    email_reset_done: 'Restored',
    email_reset_title: 'Reset to standard?',
    email_reset_text: 'This template will be restored to its standard content. Your changes will be lost.',
    email_reset_confirm: 'Reset',
    email_reset_all: 'Reset All',
    email_reset_all_title: 'Reset all templates?',
    email_reset_all_text: 'Every standard template will be restored to its standard content. All your edits will be lost.',
    email_reset_all_done: 'All templates reset to standard',
    email_modified: 'Modified',
    email_never_sent: 'Never',
    email_test_sent: 'Test email sent to',
    email_test_failed_title: 'Send failed',
    email_test_failed: 'Check your email settings.',
    email_no_account_email: 'Your account has no email address',
};
