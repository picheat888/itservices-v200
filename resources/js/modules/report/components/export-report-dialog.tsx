...
export function ExportReportDialog({
    ...
    const choices: { value: ExportFormat; title: string; desc: string; tone: string }[] = [
        { value: 'xlsx', title: t('rep_export_xlsx'), desc: t('rep_export_xlsx_desc'), tone: 'bg-emerald-500/10 text-emerald-600' },
        { value: 'pdf', title: t('rep_export_pdf'), desc: t('rep_export_pdf_desc'), tone: 'bg-red-500/10 text-red-600' },
    ];
            ...
            <DialogContent className="max-w-xl p-0">
                ...
                <div className="space-y-4 px-6 pb-2">
                    ...
                    <div className="grid gap-3 sm:grid-cols-2">
                        {choices.map((c) => (
                            <ChoiceCard key={c.value} selected={format === c.value} onClick={() => setFormat(c.value)} className="flex gap-3 rounded-lg p-3 text-left">
