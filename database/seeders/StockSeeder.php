...
class StockSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // [sku, name, serial, category, brand, model, unit, cost, current, min, max, warehouse, supplier, lastMove, warranty, trackSerial]
        // trackSerial=true → individually tracked hardware; the seeder then registers
        // one serial per on-hand unit in stock_item_serials (see seedSerials()).
        $items = [
            ['SK-NB-001', 'Lenovo ThinkPad T14 Gen 3',        null,         'แล็ปท็อป',              'Lenovo',   'ThinkPad T14 G3',   'unit',      38500,  2,  3, 10, $hq,  'Inet Solutions',                                          '2026-05-18', '3-year onsite',    true],
            ['SK-NB-002', 'Dell Latitude 5440',                null,         'แล็ปท็อป',              'Dell',     'Latitude 5440',     'unit',      32100,  5,  2,  8, $hq,  'บริษัท เดลล์ คอร์ปอเรชั่น (ประเทศไทย) จำกัด',            '2026-05-20', '3-year onsite',    true],
            ['SK-PC-001', 'Dell OptiPlex 3000 SFF',            null,         'เดสก์ท็อป',             'Dell',     'OptiPlex 3000',     'unit',      21500,  4,  2,  8, $hq,  'บริษัท เดลล์ คอร์ปอเรชั่น (ประเทศไทย) จำกัด',            '2026-05-12', '3-year onsite',    true],
            ['SK-RAM-001', 'Kingston 8GB DDR4-3200 SODIMM',    null,         'อะไหล่คอมพิวเตอร์',    'Kingston', 'KCP432SS8/8',       'module',      850, 14, 10, 40, $hq,  'Synnex Thailand',                                         '2026-05-20', 'Lifetime',         false],
            ['SK-RAM-002', 'Crucial 16GB DDR4-3200 DIMM',      null,         'อะไหล่คอมพิวเตอร์',    'Crucial',  'CT16G4DFRA32A',     'module',     1490,  6,  6, 20, $hq,  'Synnex Thailand',                                         '2026-05-15', 'Lifetime',         false],
            ['SK-SSD-001', 'Samsung 970 EVO Plus 500GB NVMe',  null,         'อะไหล่คอมพิวเตอร์',    'Samsung',  'MZ-V7S500BW',       'drive',      2100,  1,  5, 20, $hq,  'JIB Computer Group',                                      '2026-05-21', '5-year',           false],
            ['SK-MOU-001', 'Logitech M331 Silent Plus',        null,         'อุปกรณ์เสริม',          'Logitech', 'M331',              'unit',        590, 38, 10, 30, $hq,  'Office Mate',                                             '2026-04-12', '1-year',           false],
            ['SK-MOU-002', 'Dell MS116 Optical Mouse',         null,         'อุปกรณ์เสริม',          'Dell',     'MS116',             'unit',        320,  4,  8, 25, $hq,  'Office Mate',                                             '2026-02-14', '1-year',           false],
            ['SK-KB-001',  'Logitech K120 USB Keyboard',       null,         'อุปกรณ์เสริม',          'Logitech', 'K120',              'unit',        450, 22,  8, 30, $hq,  'Office Mate',                                             '2026-05-15', '1-year',           false],
            ['SK-SW-001',  'Cisco Catalyst 1300-24P PoE',      'FCW2725G123', 'สวิตช์ / เราเตอร์',    'Cisco',    'C1300-24P-4G',      'unit',      28500,  1,  1,  3, $pl1, 'Inet Solutions',                                          '2026-03-20', 'Limited Lifetime', true],
            ['SK-AP-001',  'Ubiquiti UniFi U6-Lite',           null,         'สวิตช์ / เราเตอร์',    'Ubiquiti', 'U6-Lite',           'unit',       3990,  7,  4, 15, $pl1, 'Inet Solutions',                                          '2026-05-10', '1-year',           true],
            ['SK-CB-001',  'Cat6 UTP Patch Cable 2m (Blue)',   null,         'สายเคเบิล',             'Link',     'US-5142',           'piece',        75, 85, 50, 200, $hq,  'Synnex Thailand',                                         '2026-05-18', 'No warranty',      false],
            ['SK-CB-002',  'HDMI 2.0 Cable 1.8m',              null,         'สายเคเบิล',             'Belkin',   'F3Y020bt',          'piece',       220,  0,  5, 20, $hq,  'Office Mate',                                             '2026-01-18', 'No warranty',      false],
            ['SK-UPS-001', 'APC Back-UPS BX1100C-MS 1100VA',  'AS2503312045', 'UPS',                  'APC',      'BX1100C-MS',        'unit',       4290,  3,  2,  6, $pl2, 'JIB Computer Group',                                      '2026-05-08', '2-year',           true],
            ['SK-TON-001', 'HP 26A Black LaserJet Toner',      null,         'ตลับหมึก / Toner',     'HP',       'CF226A',            'cartridge',  3200,  0,  3,  8, $hq,  'Advice IT',                                               '2026-04-30', 'No warranty',      false],
            ['SK-TON-002', 'Canon 057 Black Toner',            null,         'ตลับหมึก / Toner',     'Canon',    '057',               'cartridge',  2800,  6,  2,  6, $hq,  'Advice IT',                                               '2026-05-05', 'No warranty',      false],
        ];
        ...
        foreach ($items as $row) {
            ...
            if ($current > 0 && $item->lots()->doesntExist()) {
                $item->lots()->create([
                    'unit_cost' => $cost,
                    'qty_received' => $current,
                    'qty_remaining' => $current,
                    'received_at' => $lastMove,
                ]);
            }
        ...
        $this->seedMovements($skuMap, $costMap);
        $this->seedRequests($skuMap);
        $this->seedSerials($serialItems);
    }

    /**
     * Registers one serial per on-hand unit for each serialized SKU so the data
     * matches its current_stock. The first unit reuses the item's legacy single
     * serial when present; the rest are generated from the SKU. Idempotent.
     *
     * @param  array<string, array{id:int, current:int, serial:?string, warehouse:string, lastMove:string}>  $serialItems
     */
    private function seedSerials(array $serialItems): void
    {
        if (StockItemSerial::count() > 0) {
            return;
        }

        foreach ($serialItems as $sku => $info) {
            $base = str_replace('SK-', '', $sku);

            for ($i = 1; $i <= $info['current']; $i++) {
                $serial = ($i === 1 && ! empty($info['serial']))
                    ? $info['serial']
                    : sprintf('%s-SN%04d', $base, $i);
     ...
     * @param  array<string, float|int>  $costMap  sku => unit cost (stamped on receives)
     */
    private function seedMovements(array $skuMap, array $costMap): void
    {
        ...
        foreach ($movements as [$sku, $type, $qty, $from, $to, $ref, $by, $notes, $movedAt]) {
            ...
            StockMovement::create([
                ...
                'unit_cost' => $type === 'receive' ? ($costMap[$sku] ?? null) : null,
                'from_label' => $from,
                'to_label' => $to,
                'reference' => $ref,
                'recorded_by' => $by,
